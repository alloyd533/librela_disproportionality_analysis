...
# uncertainty quantification through sensitivity analysis. See README for more
# details
...
required_packages <- c(
  "tidyverse", "readxl", "here", "survival", "truncnorm", "viridis",
  "tictoc", "janitor", "scales", "progressr", "gt", "jsonlite", "Hmisc",
  "gtExtras", "htmltools", "future", "furrr","lhs", "ggtext"
)
...
install_if_missing("MASS") # Don't load automatically due to conflicts
...
set.seed(2612)
N_SIMS <- 1000 
# This determines how many CPU cores are used for parallel processing. If you do
# not know how many cores you have run detectCores(). Set the N_WORKERS to one 
# less than your total cores
N_WORKERS <- 8                   
CUTOFF_DATE <- ymd("2025-06-30")  

# Latin Hypercube
lhs_matrix <- randomLHS(N_SIMS, 6)

# Age distribution (scaled Beta to 1–18 years)
AGE_ALPHA <- 4.453662
AGE_BETA  <- 2.690813

# Weight distribution (lognormal from Monteiro; truncated at 100kg)
W_MEANLOG <- log(26.06)
W_SDLOG   <- 0.571740  # implied by IQR 16.0–34.6
...
# 55,966 individual PTs in 16,175 ADR reports compared to 17,162
...
life_table_by_size <- read_rds(here("data", "reference", "life_table_by_size"))
...
run_single_simulation <- function(sim_id, ae_df, reporting_rates, eu_countries, 
  ...
  underreporting_mult <- UNDERREPORTING_MIN + beta_val * (UNDERREPORTING_MAX - UNDERREPORTING_MIN)
  ...
  reporting_adj_base <- qunif(lhs_matrix[sim_id, 6], REPORTING_ADJ_MIN, REPORTING_ADJ_MAX)
  
  # Country-specific reporting rate adjustments

  reporting_rates_adjusted <- reporting_rates %>%
    mutate(
      reporting_rate_adj = reporting_adj_base * runif(n(), 0.9, 1.1),  
...
logistic_params <- empirical_uptake %>%
  ...
  summarise(
    ...
    model = list(fit_logistic_fixed_K(pick(everything()), K)),   
...
sample_age_weight_copula <- function(n, rho) {
  z <- MASS::mvrnorm(
    n = n,
    mu = c(0, 0),
  ...
  age_at_start <- 1 + 17 * qbeta(u1, shape1 = AGE_ALPHA, shape2 = AGE_BETA)
  
  weight_kg <- qlnorm(u2, meanlog = W_MEANLOG, sdlog = W_SDLOG)
  weight_kg <- pmin(weight_kg, 100)  # truncate at 100kg
...
while (cumulative_doses < target_doses_consumed) { 
  ...
  dog_demographics <- sample_age_weight_copula(batch_size, rho = correlation
  )
