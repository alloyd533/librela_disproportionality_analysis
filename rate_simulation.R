  ...
  # bedinvetmab to estimate realistic adverse drug event rates for the drug. Total
  # doses can be found on Zoetis' website (see below); demographics of treated 
  # dogs found in the Montiero study (which used internal Zoetis data) and life 
  # expectancy was estimated using survival calculations from Montoya et al.
  
  
  # Input:
  # - Cleaned and matched adverse events data (matched.csv/rds)
  # - Monteiro et al., (2025). Global pharmacovigilance reporting of the first 
  #   monoclonal antibody for canine osteoarthritis: a case study with bedinvetmab
  #   (LibrelaTM). Frontiers in Veterinary Science, 12, 1558222.
  #   https://doi.org/10.3389/FVETS.2025.1558222/BIBTEX
  # - Montoya et al., (2023). Life expectancy tables for dogs and cats derived 
  #   from clinical data. Frontiers in Veterinary Science, 10, 1082102.
  #   https://doi.org/10.3389/FVETS.2023.1082102/BIBTEX
  # - Librela for Pet Owners | Zoetis Petcare. (n.d.). Retrieved October 23, 2025,
  #   from https://www.zoetispetcare.com/products/librela?
  ...
  # Derived from Supplementary Table 8 of Montoya 2019 for size correction
  ...
  analysis_data <- ae_df %>%
    ...
    mutate(
      ...
      market_share = estimated_doses*100 / sum(estimated_doses)
    ) %>%
  ...
  # 5. Market share estimation (ADR numbers corrected by reporting rate) ---------
  ...
  empirical_uptake <- ae_df %>%
    ...
    left_join(reporting_rates %>% dplyr::select(country_group, ae_per_10k_doses), by = "country_group") %>%
    mutate(
  ...
  # 6. Uptake growth modelling ---------------------------------------------------
  # Logistic growth function with fixed addressable market
  
  # Calculate K externally (not fitted)
  calculate_K <- function(current_cumulative, months_available) {
    # Assume markets saturate in 6 years
    t_saturation <- 72
    r_assumed <- 0.1
    t0_assumed <- t_saturation / 2
    
    # Where on S-curve will each market be (based on months since authorisation)
  ...
  # 7. Simulation Functions (vectorised) -----------------------------------------
  
  # Correlated age and weight sampling using Gaussian copula
  sample_dog_age_and_weight <- function(n = 1, correlation = -0.4) {
    ...
    age_at_start <- qtruncnorm(u1, a = 1, b = 18, mean = 11.4, sd = 2.0)
    weight_kg <- qtruncnorm(u2, a = 1, b = 100, mean = 26.06, sd = 13.78)
    
    tibble(age_at_start = age_at_start, weight_kg = weight_kg)
  }
  ...
  # a. Survival months based on size and age
  survival_months_from_size_and_age_vectorized <- function(ages, size_groups) {
    ...
    # Truncate life at 18 years
    survival_months <- pmin(survival_months, 216)
    
    return(pmax(survival_months, 1))
  }
  
  # b. Treatment persistence 
  treatment_persistence_months_vectorized <- function(survival_months) {
    n <- length(survival_months)
    lambda <- 0.01  # 1% monthly discontinuation rate
    # Draw persistence duration (in months) from exponential
    treatment_months <- rexp(n, rate = lambda)
    # Dogs cannot be treated beyond their lifespan
    pmin(ceiling(treatment_months), ceiling(survival_months))
  }
  # 8. Simulation ----------------------------------------------------------------
  
  # Iteratively simulate in batches with fixed dose cut-off
  
  # Zoetis accounts and other documentation choose 30/6/2025 so assume that sold 
  # doses pertain to that date
  cutoff_date <- as_date("2025-06-30")
  known_global_doses_sold <- 30e6
  # Held in stock/wasted
  dose_buffer_pct <- 0.2
  ...
  with_progress({
    ...
    while (cumulative_doses < target_doses_consumed) {
      ...
      # Generate batch with correlated age and weight
      dog_demographics <- sample_dog_age_and_weight(batch_size, correlation = -0.4)
  ...
  write_csv(dogs_final, here("output", "simulated_dogs_with_size.csv"))
  ...
  # Plot
  dose_distributions <- dogs_final %>%
    group_by(country_group) %>%
    summarise(
      dose_dist = list(doses_by_cutoff),
      dose_dist_jit = list(doses_by_cutoff + runif(length(doses_by_cutoff), -0.4, 0.4)),
      .groups = "drop"
    )
  
  # 2) Build table data (keep list-cols as lists; add Total BEFORE gt())
  tbl <- country_summary %>%
    left_join(dose_distributions, by = "country_group") %>%
    mutate(
      Country = case_match(
        country_group,
        "EU_combined"   ~ "European Union",
      ...
      `Dogs Exposed`  = as.numeric(n_dogs),
      `Mean Estimated Doses` = as.numeric(mean_duration)
    ) %>%
    select(
      Country, `Dogs Exposed`, `Dose Distribution` = dose_dist_jit, `Mean Estimated Doses`
    ) %>%
    arrange(desc(`Dogs Exposed`)) %>%
  
  {
    pooled <- list(unlist(.$`Dose Distribution`, recursive = TRUE, use.names = FALSE))
    bind_rows(
      .,
      tibble(
        Country = "Total",
        `Dogs Exposed` = sum(.$`Dogs Exposed`, na.rm = TRUE),
        `Dose Distribution` = pooled,
        `Mean Estimated Doses` = weighted.mean(.$`Mean Estimated Doses`, w = .$`Dogs Exposed`, na.rm = TRUE)
      )
    )
  }
  
  # GT 
  simulation_table <- tbl %>%
    mutate(
      `Dogs Exposed`  = scales::comma(`Dogs Exposed`),
      `Mean Estimated Doses` = round(`Mean Estimated Doses`, 1)
    ) %>%
    gt() %>%
    gt_theme_guardian() %>%
    gt_plt_dist(
      column = `Dose Distribution`,
      type = "density",
      same_limit = TRUE,     # common axes (uses pooled limits incl. Total row)
      fill = "#9b4b16"
    ) %>%
    tab_header(
      title = md("**Individual Dog Simulation**"),
      subtitle = md("_Estimated bedinvetmab exposure and dose distribution_")
    ) %>%
    opt_table_font(font = list(google_font("Merriweather"), default_fonts())) %>%
    tab_options(
      table.background.color = "#fff1e5",
      heading.background.color = "#f2e6dd",
      column_labels.background.color = "#f2e6dd",
      row.striping.background_color = "#f8f3ef",
      data_row.padding = px(6)
    ) %>%
    cols_align(align = "left", columns = Country) %>%
    cols_align(align = "right", columns = c(`Dogs Exposed`, `Mean Estimated Doses`)) %>%
    tab_style(
      style = list(cell_fill(color = "#f2e6dd"), cell_text(weight = "bold")),
      locations = cells_body(rows = Country == "Total")
    )
  
  simulation_table
  
  gtsave(simulation_table, here("output", "tables", "simulation_table.html"))
  
  # 10. Calculate ADR rates ------------------------------------------------------
  
  # Aggregate ADRs (numerator)
  adr_summary <- ae_df %>%
  ...
  adr_top10_clean <- adr_rates %>%
    ...
    mutate(
      ...
      # order x-axis by descending number of simulated dogs
      country_group = fct_reorder(country_group, n_exposed, .desc = TRUE),
  ...
  adr_rate_plot <- adr_top10_clean %>%
    mutate(
      ...
      rate_pct = rate_per_10000 / 100
    ) %>%
    ggplot(aes(x = country_group, y = pt, fill = rate_pct)) +
    geom_tile(color = "#e5d9cc", linewidth = 0.4) +
    geom_text(
